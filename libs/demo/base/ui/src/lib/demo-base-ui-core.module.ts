...
export class DemoBaseUiCoreModule {
  private logger: LoggerService = inject<LoggerService>(LoggerService);

  constructor(
    @Optional()
    @SkipSelf()
    parentModule: DemoBaseUiCoreModule
  ) {
    if (parentModule) {
      throw new Error(
        'DemoBaseUiCoreModule is already loaded. Import it in the AppModule only.'
      );
    }
    this.init();
  }
  ...
  private init(): void {
    this.logger.console.debug('DemoBaseUiCoreModule', 'init');
  }
}
